...
abstract class ScanBlueToothBaseActivity : PdfBaseActivity() {
    ...
    fun showBlueToothDevice(returnAction: (Boolean) -> Unit) {
        ...
        val btnClose: AppCompatImageButton = dialog.findViewById<View>(R.id.btn_close) as AppCompatImageButton
...
*/
    ...
    /*private fun listView(linear: LinearLayoutCompat, macSelect: String, reload: (String) -> Unit) {
        for(bt in mDeviceList) {
            linear.addView(itemInflateView(macSelect, bt, linear) {device ->
                //dialog.dismiss()
                initPrint(PapersManager.macPrint, testPrint() as ArrayList<CloseCartResponse.ClientVoucher>)
                toast("Impresora seleccionada")
    ...
    private fun testPrint() : MutableList<CloseCartResponse.ClientVoucher> {
        ...
        return featuresJsonEntity?.features as MutableList<CloseCartResponse.ClientVoucher>
    }

    private fun itemInflateView(mac: String, data: BlueToothDeviceData, linear: LinearLayoutCompat, returnAction: (String) -> Unit): View {
